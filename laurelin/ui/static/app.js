...
function getToken() {
  try { return localStorage.getItem("laurelin_token") || ""; } catch { return ""; }
}
...
function errorBox(err) {
  ...
  if (err && err.status === 401) {
    box.append(
      h("div", {}, "401 Unauthorized — this server requires an API token (LAURELIN_TOKEN)."),
      h("div", { class: "token-form" },
        h("input", { type: "password", placeholder: "Paste bearer token…", id: "token-input" }),
        h("button", {
          class: "small",
          onClick: () => {
            const val = document.getElementById("token-input").value.trim();
            try { localStorage.setItem("laurelin_token", val); } catch { /* private mode */ }
            loadWorkspaceInfo();
            render();
          },
        }, "Save token & retry"),
...
async function renderPipeline() {
  ...
  wrap.append(h("h2", {}, "Builds"), h("div", { class: "toolbar" }, buildBtn, buildMsg));
